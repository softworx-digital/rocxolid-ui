...
export default function () {
    ...
    Array.isArray(options.contentStyles) && $.each(options.contentStyles, function (i, style) {
        let idStr = style.id || '';
    ...
    // add custom sripts
    let scripts = '';
    Array.isArray(options.contentScripts) && $.each(options.contentScripts, function (i, script) {
        let idStr = script.id || '';

        if (script.src) {
            scripts += `<script type="text/javascript" src="${script.src}" ${idStr} />\n`;
        } else {
            scripts += `<script type="text/javascript" ${idStr}>${script.content}</script>\n`;
        }
    });
    self.iframeBody.append(scripts);
